...
public abstract class AbstractTestTimeSource extends AbstractKiekerTest { // NOPMD (no abstract methods)
  ...
  */
   ...
   */
  public final void testTimestamping(final ITimeSource timesource, final TimeUnit timeunit, final long offset) { // NOPMD (only used by other tests)
    ...
    // has to be detected.
    for (int i = 0; i < 2; i++) {
      final long before = System.currentTimeMillis() - offset - timeSourceAbsToleranceMillis;
      final long measured = timesource.getTime();
      final long after = System.currentTimeMillis() - offset + timeSourceAbsToleranceMillis;
