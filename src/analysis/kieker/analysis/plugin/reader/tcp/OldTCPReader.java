...
@Plugin(description = "A reader which reads records from a TCP port",
		outputPorts = {
			@OutputPort(name = OldTCPReader.OUTPUT_PORT_NAME_RECORDS, eventTypes = { IMonitoringRecord.class }, description = "Output Port of the OldTCPReader")
		},
		configuration = {
			@Property(name = OldTCPReader.CONFIG_PROPERTY_NAME_PORT1, defaultValue = "10133",
					description = "The first port of the server used for the TCP connection."),
			@Property(name = OldTCPReader.CONFIG_PROPERTY_NAME_PORT2, defaultValue = "10134",
					description = "The second port of the server used for the TCP connection.")
		})
public class OldTCPReader extends AbstractReaderPlugin {
	...
	public OldTCPReader(final Configuration configuration, final IProjectContext projectContext) {
	 ...
	 * Used for tests
	 */
	public OldTCPReader(final Configuration configuration, final IProjectContext projectContext, final ServerSocketChannelFactory serverSocketChannelFactory) {
		super(configuration, projectContext);
	...
	public void terminate(final boolean error) {
		this.log.info("Shutdown of " + this.getClass().getName() + " requested.");
